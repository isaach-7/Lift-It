...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  const callback = pathname === '/auth/callback'
  return (
        ...
        {state.status === 'error' && (
          <>
            ...
            <p className="feedback error" role="alert">
              {callback
                ? 'This sign-in link may have expired or already been used. Request a new link, or try again if your connection failed.'
                : 'We could not check your session. Check your connection and try again.'}
            </p>
            <a className="text-link" href={callback ? '/login' : pathname}>
